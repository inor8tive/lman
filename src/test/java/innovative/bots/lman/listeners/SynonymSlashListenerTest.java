...
@ActiveProfiles("test")
public class SynonymSlashListenerTest {
    @InjectMocks
    SynonymsSlashCommandListener synonymsSlashCommand;
    ...
    void on_slash_only_uses_word_if_only_single_word_provided() {
        ...
        when(mockSlashCommand.getName()).thenReturn("synonyms");
        when(mockSlashCommand.getOption("word")).thenReturn(mockOption);

        List<String> ss = Arrays.asList("dog", "another", "related");

        when(languageService.getSynonyms("energy")).thenReturn(ss);
        when(messageBuilderService.buildStringList(ss, "Synonyms of the word 'ENERGY'")).thenReturn("message");
        synonymsSlashCommand.onSlashCommand(mockSlashCommand);
    }

    @Test
    void on_slash_sends_message_of_synonyms() {
        ReplyAction mockAction = mock(ReplyAction.class);
        InteractionHook mockInteractionHook = mock(InteractionHook.class);
        WebhookMessageAction<Message> mockWebhook  = mock(WebhookMessageAction.class);
        SlashCommandEvent mockSlashCommand = mock(SlashCommandEvent.class);
        ...
        when(languageService.getSynonyms("dog")).thenReturn(ss);
        when(messageBuilderService.buildStringList(ss, "Synonyms of the word 'DOG'")).thenReturn("message");
        synonymsSlashCommand.onSlashCommand(mockSlashCommand);
    }
